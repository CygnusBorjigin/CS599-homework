...
Theorem plus_n_Sm : forall n m : nat,
  S (n + m) = n + (S m).
Proof. intros n m. induction n as [|a].
- simpl. reflexivity.
...
Proof. intros n. simpl. induction n as [|a].
  - simpl. reflexivity.
  - simpl. rewrite -> IHa. destruct (even a) as [|b] eqn:E.
    + simpl. reflexivity.
    + simpl. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - rewrite <- plus_1_l. simpl. rewrite <- plus_1_l.
    assert (H: n * (1 + a) = n + n * a).
    { simpl. rewrite <- mult_n_Sm. rewrite -> add_comm. reflexivity. }
    rewrite -> H. rewrite -> IHa. reflexivity.
Qed. 
