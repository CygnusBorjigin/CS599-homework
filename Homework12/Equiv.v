...
Proof.
  ...
  - (* -> *)
    ...
    + (* b evaluates to false (contradiction) *)
      subst. unfold bequiv in Hb. simpl in Hb.
      rewrite Hb in H5.
      discriminate.
  - (* <- *)
...
Proof.
  ...
  - intros H2.
    ...
    rewrite <- H3.
    clear H0 H3.
    assert (H3: st =[x := a]=> (x !-> aeval st a ; st)).
    { apply E_Asgn. reflexivity. }
    unfold aequiv in H1.
    rewrite <- H1 in H3.
    simpl in H3.
...
Theorem CIf_congruence : forall b b' c1 c1' c2 c2',
  ...
  cequiv <{ if b then c1 else c2 end }>
         <{ if b' then c1' else c2' end }>.
Proof.
  intros b1 b2 c1 c2 d1 d2 h1 h2 h3.
   split.
  - intros H1.
    inversion H1. subst.
    + unfold bequiv in h1.
    ...
    + subst. unfold bequiv in h1.
      rewrite h1 in H5.
      apply E_IfFalse.
      * assumption.
      * unfold cequiv in h3.
        apply h3 in H6. assumption.
