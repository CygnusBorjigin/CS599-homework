...
(** ... but this would quickly become tedious, partly because we
    ...
    mostly because we would also need to define new versio ns of all
...
Proof.
  intros l m n A.
  induction m as [|a b c].
  - simpl. reflexivity.
  - simpl. rewrite -> c. reflexivity. Qed.

Lemma app_length : forall (X:Type) (l1 l2 : list X),
  length (l1 ++ l2) = length l1 + length l2.
Proof.
  intros l1 l2 X.
  induction l2 as [| a b c].
  - reflexivity.
  - simpl. rewrite -> c. reflexivity. Qed.
(** [] *)
...
Proof.
  ...
  - simpl. rewrite -> c. rewrite -> app_assoc. reflexivity. Qed.

Theorem rev_involutive : forall X : Type, forall l : list X,
  rev (rev l) = l.
Proof.
  intros l X.
  induction X as [| a b c].
  - reflexivity.
  - simpl. rewrite -> rev_app_distr. simpl. rewrite -> c. reflexivity. Qed.
(** [] *)
...
Fixpoint split {X Y : Type} (l : list (X*Y)) : (list X) * (list Y) :=
  match l with
  | nil => (nil, nil)
  | h :: t => ((fst h) :: (fst (split t)), (snd h) :: (snd (split t))) end.
...
Compute (fun x=> even x && negb(leb x 7)) 8.
Compute (fun x=> even x && negb(leb x 7)) 6.
Compute (fun x=> even x && negb(leb x 7)) 9.
...
Lemma dis_map : forall (X Y : Type)(f: X -> Y) (x y : list X),
  map f (x ++ y) = (map f x) ++ (map f y).
Proof. intros X Y f x y.
  induction x as [|a b c].
  - simpl. reflexivity.
  - simpl. rewrite -> c. reflexivity. Qed.
...
Proof.
  intros X Y f l.
  induction l as [| a b c].
  - simpl. reflexivity.
  - simpl. rewrite <- c.
    assert(H: [f a] = map f [a]).
    { simpl. reflexivity. }
    rewrite -> H.
    rewrite -> dis_map. reflexivity. Qed.
...
Compute fold_map (fun x => x + 1) [1;2;3].
Compute fold_map (fun x => x * 3) [6;7;8].
...
(* In coq, when you write fun (x : T) => _, when this occures, it is always the case
  that "T : Type" *)

Definition succ (n : cnat) : cnat
  := fun (X: Type) (f: X -> X) (x : X) => f (n f x).
