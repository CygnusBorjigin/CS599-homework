...
Proof.
  ...
  - inversion H2.
  - inversion H2.
  - inversion H2. subst. simpl in H. discriminate H.
  - inversion H2. subst. apply IHceval2. apply H2.
Qed.
...
Inductive no_whilesR: com -> Prop :=
  ...
  | nw_asign (s : string) (a : aexp): no_whilesR (CAsgn s a)
  | nw_seq (c1 : com) (c2 : com) : no_whilesR (CSeq c1 c2)
  | nw_if (b: bexp) (c1 : com) (c2 : com) : no_whilesR (CIf b c1 c2)
.
...
Proof.
  ...
  intros c.
  split.
  - intros H1. induction c.
    + apply nw_skip.
    + constructor.
    + constructor.
    + constructor.
    + inversion H1.
  - intros H1. destruct c.
    + reflexivity.
    + reflexivity.
    + 
  (* FILL IN HERE *) Admitted.
...
Fixpoint s_execute (st : state) (stack : list nat)
  ...
  | [] => stack
  | h::t => match h with
            | SPush ele => s_execute st (ele :: stack) t
            | SLoad ele => s_execute st ((st ele) :: stack) t
            | SPlus => match stack with
                       | stack_h1::stack_h2::stack_t => s_execute st ((stack_h1 + stack_h2) :: stack_t) t
                       | _ => [] end
            | SMinus => match stack with
                       | stack_h1::stack_h2::stack_t => s_execute st ((stack_h2 - stack_h1) :: stack_t) t
                       | _ => [] end
...
Fixpoint s_compile (e : aexp) : list sinstr :=
  ...
  | ANum ele => [SPush ele]
  | AId ele => [SLoad ele]
  | APlus ele1 ele2 => (s_compile ele1) ++ (s_compile ele2) ++ [SPlus]
  | AMinus ele1 ele2 => (s_compile ele1) ++ (s_compile ele2) ++ [SMinus]
  | AMult ele1 ele2 => (s_compile ele1) ++ (s_compile ele2) ++ [SMult] end.
