...
(* For the inputed state st, the value of X evaluated using st
   is lesser than or equal to the value of Y evaluated using st *)

Definition assn2 : Assertion :=
  fun st => st X = 3 \/ st X <= st Y.

(* For the inputed state st, the value if X evaluated using st
   is equal to 3 OR the value of X evaluated using st is lesser than
   or equal to the value of Y evaluatedusing st *)
...
(* 
   For the inputed state st, the value of Z evaluated using st
   multipled the value of Z evaluated using st is less than
   the value of X evaluated using st. 
   
   AND

   the successor of the value of Z evaluated using st multiplied
   by the successor of the value of Z evaluated using st is not 
   lesser than or equal to the value of X evaluated using st.
...
(*
  For the inputed state st, the valueof Z evaluated using st is equal
  to the maximumvalue between the value of X evaluated using st and 
  the value of Y evaluated using st.
*)
...
(** **** Exercise: 1 star, standard, optional (triples)
    ...
    Paraphrase the following in English.
     ...
     1) {{True}} c {{X = 5}}
      ...
      conclusion : the state in whcih the command [c] terminates in 
                   satisfy the assertion X = 5
     ...
     3) {{X <= Y}} c {{Y <= X}}
        ...
        assumption 1 : the command [c] start executing in a state where
                       the value of X is lesser than or equal to the
                        value of Y.

        assumption 2 : the command [c] eventually terminates in a state

        conclusion : the state in which the command [c] terminates in
                     satisfy the assertion where is value of Y is lesser
                      than or equal to the value of X.

     4) {{True}} c {{False}}

        assumption 1 : the command [c] start executing in a state that
                       satisfy the assertion Ture.

        assumption 2 : the command [c] eventually terminates in a state.

        conclusion : the state in which the command [c] terminates in
                      satisfy the assertion False.
     ...
     5) forall m,
        ...
        assumption 2 : the command [c] eventuall terminates in a state.

        conclusion : the state in which the command [c] terminates in
                      satisfy the assertion Y = real_fact m for all
                      m inputed.
     ...
     6) forall m,
        ...
        assumption 2 : the command [c] eventually terminatesin a state

        conclusion : the state in which the command [c] terminates in
                     satisfy the assertion:
                     the value of Z evaluated using that state multiply
                     the value of Z evaluated usingthat state result in
                     a value that is lesser than or equal to m for all
                     m inputed
                     AND
                     the successor of the value of Z evaluated using that
                     state multiply by the successor of the value of Z 
                     evaluated using that state result in a value that is
                     not lesser than or equal to m for all m inputed.
