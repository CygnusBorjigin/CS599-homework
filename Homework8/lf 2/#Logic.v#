...
Proof.
  ...
  - destruct m.
    + reflexivity.
    + Search(_+_). rewrite -> PeanoNat.Nat.add_comm in H. discriminate.
Qed.
...
Proof.
  ...
  destruct n.
  - left. reflexivity.
  - right. Search(mult). simpl in H. apply and_exercise in H.
    destruct H. apply H.
Qed.
...
Proof.
  ...
  - intros [H1 | H2].
    ...
    + destruct H2. split. right. apply H. right. apply H0.
  - intros [H1  [H2 | H3]].
    + left. apply H2.
    + destruct H1 as [H4 | H5].
      * left. apply H4.
      * right. split. apply H5. apply H3.
Qed.
...
Proof.
  intros P x H1 H2. unfold not in H2. destruct H2 as [z H3].
...
Proof.
    intros X P Q.
    split.
    - intros [x H1]. destruct H1 as [H2 | H3].
...
Proof.
  intros A B f l y.
  split.
  - intros H1. induction l as [| a b IH1].
    + simpl in H1. destruct H1.
    + simpl in H1. destruct H1.
      * exists a. split.
        ** apply H.
        ** left. reflexivity.
      * apply IH1 in H. destruct H. destruct H as [H1 H2].
        exists x. split.
        ** apply H1.
        ** simpl. right. apply H2.
  - intros H1. destruct H1. induction l as [| a b IH1].
    + simpl. simpl in H. destruct H. destruct H0.
    + simpl. simpl in H. destruct H. destruct H0.
      * left. rewrite H0. apply H.
      * right. apply IH1. split.
        ** apply H.
        ** apply H0.
Qed.
(** [] *)
...
Proof.
  intros A l. split.
  - intros H1. induction l as [| x y IH].
...
Compute All is_three [2; 4; 6].
Compute All is_three [3; 3; 3].
...
Proof.
   intros T P l.
    split.
    - intros H. induction l as [| a b IH].
      + simpl. reflexivity.
      + simpl. split.
        * apply H. simpl. left. reflexivity.
        * apply IH. simpl in H. intros x H1. apply H. right. apply H1.
    - intros H1. induction l as [| a b IH].
      + simpl. intros x H2. destruct H2.
      + simpl. intros x H2. destruct H1 as [H1 H3]. destruct H2 as [H2 | H2].
        * rewrite <- H2. apply H1.
        * apply IH.
          ** apply H3.
          ** apply H2.
...
Compute rev_append [1;2;3;4;5] [9;10;11].

Definition tr_rev {X} (l : list X) : list X :=
  rev_append l [].

Compute tr_rev [1;2;3;4].
...
Proof.
  ...
  - simpl. destruct x0.
    + simpl. reflexivity.
    + simpl. rewrite <- app_assoc. simpl. destruct x1.
      * simpl. reflexivity.
      * simpl. rewrite <- app_assoc. simpl.
      (H1: rev_append (x0::x1) [] = rev (x0::x1) -> rev_append x1 [x0] = rev x1 ++ x0)


  intros x.
  induction x as [| a b IH].
  - simpl. unfold tr_rev. simpl. reflexivity.
  - simpl. rewrite <- IH. unfold tr_rev. simpl.
    assert (H1: b = b ++ []).
    { Search(_++_). rewrite app_nil_r. reflexivity. }
    assert (H2: [] ++ [a] = [a]).
    { reflexivity. }
    assert (H3: rev_append b [] = rev b).
    { rewrite <- IH. unfold tr_rev. reflexivity. }*)
Admitted.
...
Proof.
  ...
  - Search(even). rewrite even_S. destruct (even a).
    + simpl. destruct b. exists x. rewrite <- H. reflexivity.
    + simpl. destruct b. exists (S x). rewrite -> H. simpl. reflexivity.
Qed.
(** [] *)
...
Fixpoint eqb_list {A : Type} (eqb : A -> A -> bool)
  ...
  | h::t, [] => false
  | [], h::t => false
  | h1::t1, h2::t2 => if eqb h1 h2 then eqb_list eqb t1 t2 else false end.
...
Proof.
  intros A eqb H1. split.
  - intros H2. induction l1 as [| a b IH].
    + unfold eqb_list in H2. destruct l2.
      * reflexivity.
      * discriminate H2.
...
Proof.
  ...
  assert(H1: P -> ~P -> False).
  { intros H1 H2. unfold not in H2. apply H2. apply H1. }
  apply H.
